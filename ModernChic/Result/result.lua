...
local skinProperty = require("lua.require.result.skinproperty")
local property = require("lua.require.result.resultproperty")
local filepath = require("lua.require.result.filepath")
local offset = require("lua.require.result.offset")

local header = {
	type = 7,			-- 0:7k, 1:5k, 2:12k, 3:10k, 4:9k, 5:select, 6:decide, 7:result, 15:courceresult, 16:24k, 17:24kDouble
...
local function main()
	...
	skin.source =  {
		...
		{id = 8, path = "parts/bg/result/clear/*.png"},
		{id = 9, path = "parts/bg/result/failed/*.png"},
		{id = 10, path = "parts/bg/result/all/*.png"},
	...
	do
		...
		local bottom = main_state.text(12) .." / " ..main_state.text(14) .." / " ..main_state.text(13) .." / " ..main_state.text(1003)
	...
	do
		...
		local status, parts = pcall(function()
			return dofile(path).load(mainPosX, 0)
		end)
