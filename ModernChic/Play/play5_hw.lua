...
local function is5keyLanecoverOff()
	return skin_config.option["5鍵用レーンカバー（5鍵モード時のみ）"] == 995
end
local function is5keyLanecoverOn()
	return skin_config.option["5鍵用レーンカバー（5鍵モード時のみ）"] == 996
end

local header = {
	type = 1,			-- 0:7k, 1:5k, 2:12k, 3:10k, 4:9k, 5:select, 6:decide, 7:result, 15:courceresult, 16:24k, 17:24kDouble
...
local function main()
	...
	local fivekeyCoverPosX
	local fivekeyCoverAngle
	if is_left_scratch() then
		keyPosition = {114, 177, 228, 291, 342, 3}
		keyflashPosX = {110, 167, 224, 281, 338}
		scratchflashPosX = 5
		fivekeyCoverPosX = 405
		fivekeyCoverAngle = 180
	elseif is_right_scratch() then
		keyPosition = {117, 180, 231, 294, 345, 408}
		keyflashPosX = {113, 170, 227, 284, 341}
	...
	do
		...
		skin.image = {
			...
			-- 5鍵用カバー
			{id = "5keysFrame", src = 1, x = 1760, y = 0, w = 114, h = 900},
		...
		skin.note = {
			...
			dst = {
				...
				-- 皿
				{x = playsidePositionX + keyPosition[6], y = notesPositionY - 12, w = scWidth, h = 857},
			},
	...
	-- 進捗バー
	do
		local progress_path = skin_config.get_path("lua/progress.lua")
	...
	-- キー入力
	do
		local inputkey7_path = skin_config.get_path("lua/sp/inputkey5.lua")
	...
	-- 5鍵用カバー
	if is5keyLanecoverOn() then
		table.insert(skin.destination,{
			id = "5keysFrame", loop = 1500, dst = {
				{time = 0, x = playsidePositionX + fivekeyCoverPosX, y = 1080, w = 114, h = 900, acc = 2, a = 240, angle = fivekeyCoverAngle},
				{time = 1000},
	...
	-- 爆発
	do
		local bomb7_path = skin_config.get_path("lua/sp/bomb5.lua")
