...
local function load(playsidePositionX, notesPositionY)
	...
	parts.image = {}
	do
		local init = {"s", "1", "2", "3", "4", "5", "6", "7"}
		local bombTimer = {50, 51, 52, 53, 54, 55, 56, 57}
		local lnBombTimer = {70, 71, 72, 73, 74, 75, 76, 77}
		local modernchicLnPosY = {}
		local oadxLnPosY = {}
		-- 判定ボムを使う場合はLNボム色を同じものにする
		if isJudgeTimingBombOff() then
			modernchicLnPosY = {900, 300, 600, 300, 600, 300, 600, 300}
			oadxLnPosY = {576, 192, 384, 192, 384, 192, 384, 192}
		elseif isJudgeTimingBombOn() then
			modernchicLnPosY = {0, 0, 0, 0, 0, 0, 0, 0}
			oadxLnPosY = {0, 0, 0, 0, 0, 0, 0, 0}
		end
	...
	-- 爆発エフェクト
	do
		local init = {"1", "2", "3", "4", "5", "6", "7", "s"}
		-- 通常ボムタイマ
		local n_timer = {51, 52, 53, 54, 55, 56, 57, 50}
		-- LN用ボムタイマ
		local ln_timer = {71, 72, 73, 74, 75, 76, 77, 70}
		local posX = {}
		...
		if isModernChicBomb() then
			...
			if is_left_scratch() then
				posX = {142, 197, 257, 312, 372, 427, 487, 55}
			elseif is_right_scratch() then
				posX = {30, 85, 144, 200, 260, 315, 375, 459}
			end
		elseif isOADXBomb() then
			-- ボムの大きさ調整
			...
			-- ボム中心点
			if is_left_scratch() then
				posX = {167, 222, 282, 337, 397, 452, 512, 80}
			elseif is_right_scratch() then
				posX = {55, 110, 169, 225, 285, 340, 400, 484}
			end
		end
