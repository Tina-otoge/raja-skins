...
local function load(playsidePositionX, notesPositionY)
	...
	do
		...
		-- キーが押されたとき-----------------------------------------------------
		do
			local timer = {101, 102, 103, 104, 105, 100}
			for i = 1, 5, 1 do
			...
			-- 皿
			table.insert(parts.destination,	{
				id = "keybeam-"..init[6], offset = 3, timer = timer[6], blend = 1, loop = -1, op = {32}, dst = {
					{time = 0, x = playsidePositionX + posX[6], y = notesPositionY, w = width[6], h = 0},
					{time = 50, h = keybeam_height},
					{time = 85},
					{time = 170, x = playsidePositionX + posX[6] + 54, w = 0, a = 0}
				}
			})
			-- 皿（オートプレイ時）
			table.insert(parts.destination,	{
				id = "keybeam-"..init[6], offset = 3, timer = timer[6], blend = 1, op = {33}, dst = {
					{x = playsidePositionX + posX[6], y = notesPositionY, w = width[6], h = keybeam_height}
				}
			})
		end
		...
		do
			local timer = {121, 122, 123, 124, 125, 120}
			...
			if isBeamDisappearanceTypeL() then
				...
				-- 皿はオートプレイのみ適用
				table.insert(parts.destination,	{
					id = "keybeam-"..init[6], offset = 3, timer = timer[6], blend = 1, loop = time_keyoff, op = {965, 33}, dst = {
						{time = 0, x = playsidePositionX + posX[6], y = notesPositionY, w = width[6], h = keybeam_height, acc = 2},
						{time = time_keyoff, a = 0}
					}
				})
			elseif isBeamDisappearanceTypeB() then
				...
				-- 皿はオートのみ適用
				table.insert(parts.destination,	{
					id = "keybeam-"..init[6], offset = 3, timer = timer[6], blend = 1, loop = time_keyoff, op = {966, 33}, dst = {
						{time = 0, x = playsidePositionX + posX[6], y = notesPositionY, w = width[6], h = keybeam_height, acc = 2},
						{time = time_keyoff, x = playsidePositionX + move_x[6], w = 0, a = 0}
					}
				})
			end
		end
