...
local function load(laneLeftPosX, laneRightPosX, notesPositionY)
	...
	do
		-- 鍵盤の種類
		local init = {"w", "b", "w", "b", "w", "s"}
		-- 配置位置
		local keyLeftPosX = {114, 177, 228, 291, 342, 3}
		local keyRightPosX = {117, 180, 231, 294, 345, 407}
		-- 横幅
		local width = {60, 48, 60, 48, 60, 108}
		-- キービームの長さ
		local keybeam_height = setKeybeamHeight()
		
		-- キーが押されたとき-----------------------------------------------------
		do
			local keyLeftTimer = {101, 102, 103, 104, 105, 100}
			local keyRightTimer = {111, 112, 113, 114, 115, 110}
			...
			-- 左皿
			table.insert(parts.destination,	{
				id = "keybeam-"..init[6], offset = 3, timer = keyLeftTimer[6], blend = 1, loop = -1, op = {32}, dst = {
					{time = 0, x = laneLeftPosX + keyLeftPosX[6], y = notesPositionY, w = width[6], h = 0},
					{time = 50, h = keybeam_height},
					{time = 85},
					{time = 170, x = laneLeftPosX + keyLeftPosX[6] + 54, w = 0, a = 0}
				}
			})
			-- 右皿
			table.insert(parts.destination,	{
				id = "keybeam-"..init[6], offset = 3, timer = keyRightTimer[6], blend = 1, loop = -1, op = {32}, dst = {
					{time = 0, x = laneRightPosX + keyRightPosX[6], y = notesPositionY, w = width[6], h = 0},
					{time = 50, h = keybeam_height},
					{time = 85},
					{time = 170, x = laneRightPosX + keyRightPosX[6] + 54, w = 0, a = 0}
				}
			})
			-- 左皿（オートプレイ時）
			table.insert(parts.destination,	{
				id = "keybeam-"..init[6], offset = 3, timer = keyLeftTimer[6], blend = 1, op = {33}, dst = {
					{x = laneLeftPosX + keyLeftPosX[6], y = notesPositionY, w = width[6], h = keybeam_height}
				}
			})
			-- 右皿（オートプレイ時）
			table.insert(parts.destination,	{
				id = "keybeam-"..init[6], offset = 3, timer = keyRightTimer[6], blend = 1, op = {33}, dst = {
					{x = laneRightPosX + keyRightPosX[6], y = notesPositionY, w = width[6], h = keybeam_height}
				}
			})
		end
		...
		-- timer 130 ~ 2P同様
		do
			local leftTimer = {121, 122, 123, 124, 125, 120}
			local rightTimer = {131, 132, 133, 134, 135, 130}
			...
			if isBeamDisappearanceTypeL() then
				...
				-- 皿はオートプレイのみ適用
				table.insert(parts.destination,	{
					id = "keybeam-"..init[6], offset = 3, timer = leftTimer[6], blend = 1, loop = time_keyoff, op = {33}, dst = {
						{time = 0, x = laneLeftPosX + keyLeftPosX[6], y = notesPositionY, w = width[6], h = keybeam_height, acc = 2},
						{time = time_keyoff, a = 0}
					}
				})
				table.insert(parts.destination,	{
					id = "keybeam-"..init[6], offset = 3, timer = rightTimer[6], blend = 1, loop = time_keyoff, op = {33}, dst = {
						{time = 0, x = laneRightPosX + keyRightPosX[6], y = notesPositionY, w = width[6], h = keybeam_height, acc = 2},
						{time = time_keyoff, a = 0}
					}
				})
			elseif isBeamDisappearanceTypeB() then
				-- TYPE-B
				local movePosxLeft = {144, 207, 258, 321, 372, 57}
				local movePosxRight = {147, 204, 261, 318, 375, 461}
				...
				table.insert(parts.destination,	{
					id = "keybeam-"..init[6], offset = 3, timer = leftTimer[6], blend = 1, loop = time_keyoff, op = {33}, dst = {
						{time = 0, x = laneLeftPosX + keyLeftPosX[6], y = notesPositionY, w = width[6], h = keybeam_height, acc = 2},
						{time = time_keyoff, x = laneLeftPosX + movePosxLeft[6], w = 0, a = 0}
					}
				})
				table.insert(parts.destination,	{
					id = "keybeam-"..init[6], offset = 3, timer = rightTimer[6], blend = 1, loop = time_keyoff, op = {33}, dst = {
						{time = 0, x = laneRightPosX + keyRightPosX[6], y = notesPositionY, w = width[6], h = keybeam_height, acc = 2},
						{time = time_keyoff, x = laneRightPosX + movePosxRight[6], w = 0, a = 0}
					}
				})
			end
		end
