...
local function load(laneLeftPosX, laneRightPosX, notesPositionY)
	...
	parts.image = {}
	do
		local init = {"sl", "1", "2", "3", "4", "5", "sr", "6", "7", "8", "9", "10"}
		local bombTimer = {50, 51, 52, 53, 54, 55, 60, 61, 62, 63, 64, 65}
		local lnBombTimer = {70, 71, 72, 73, 74, 75, 80, 81, 82, 83, 84, 85}
		local modernchicLnPosY = {}
		local oadxLnPosY = {}

		-- 判定ボムを使う場合はLNボム色を同じものにする
		if isJudgeTimingBombOff() then
			modernchicLnPosY = {900, 300, 600, 300, 600, 300, 900, 300, 600, 300, 600, 300}
			oadxLnPosY = {576, 192, 384, 192, 384, 192, 576, 192, 384, 192, 384, 192}
	...
	do
		local num = 6
		local initLeft = {"1", "2", "3", "4", "5", "sl"}
		local initRight = {"6", "7", "8", "9", "10", "sr"}
		-- 通常ボムタイマ
		local normalTimerLeft = {51, 52, 53, 54, 55, 50}
		local normalTimerRight = {61, 62, 63, 64, 65, 60}
		-- LN用ボムタイマ
		local lnTimerLeft = {71, 72, 73, 74, 75, 70}
		local lnTimerRight = {81, 82, 83, 84, 85, 80}
		...
		if isModernChicBomb() then
			...
			-- ボム中心点
			leftPostionX = {142, 197, 257, 312, 372, 55}
			rightPostionX = {144, 200, 260, 315, 375, 459}
		elseif isOADXBomb() then
			-- ボムの大きさ調整
